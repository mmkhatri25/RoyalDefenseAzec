...
public class Effect_Hit_Display : MonoBehaviour
{
	public bool test;
	...
	public void HitEffectProperies(int toggle, GameObject target, int effectClass, int effectNumber)
	{
		if (toggle == 1)
		{
			targetObject = target;
	...
	private void ResetEffect()
	{
		if (randomPosition == 1)
		{
			image.transform.localPosition = new Vector3(UnityEngine.Random.Range(-0.2f, 0.2f), UnityEngine.Random.Range(-0.2f, 0.2f), -1.2f);
		}
		else
		{
			image.transform.localPosition = new Vector3(0f, 0f, -1.2f);
		}
		Transform transform = image.transform;
		Vector3 position = image.transform.position;
		float x = position.x;
		Vector3 position2 = image.transform.position;
		transform.position = new Vector3(x, position2.y, -1.2f);
		if (randomRotation == 1)
		{
